...
export class VuexPersistence<S> implements PersistOptions<S> {
  ...
  public constructor(options?: PersistOptions<S>) {
    ...
    const storageConstructor = this.storage && this.storage.constructor && this.storage.constructor.name && this.storage.constructor.name.toLowerCase()
        ...
        (options.saveState != null)
          ...
          : ((key: string, state: {}, storage: AsyncStorage) =>
              (storage).setItem(
                ...
                (((storage && storage.constructor && storage.constructor.name && storage.constructor.name.toLowerCase()) === 'localforage')
                    ? merge({}, state || {})
                    : (
                      this.supportCircular
